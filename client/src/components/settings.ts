...
import Dropdown, { DropdownButton, DropdownParameters } from './input/focus/dropdown';
...
class SettingField extends Component<'fieldset'> {
    ...
    constructor({ labelText, settingId, onupdate, ...params }: SettingParameters) {
        super({
            tag: 'fieldset',
            ...params,
        });
...
interface SettingsPanelParameters
    extends Omit<Omit<DropdownParameters<'form', 'fieldset'>, 'tag'>, 'options'> {
    ...
    constructor({ onupdate, ...params }: SettingsPanelParameters) {
        ...
        this.element.onsubmit = async (ev) => {
            ...
            let r = await updateSettings({
                matchmakingCoefficient: Number(this.matchmaking.input.element.value),
            });
...
export class SettingsButton extends DropdownButton<'form', 'fieldset'> {
    constructor() {
        ...
        this.element.style.backgroundColor = 'var(--celadon)';
        this.element.classList.value = 'icon-button settings-button dropdown-button';
    }
}
