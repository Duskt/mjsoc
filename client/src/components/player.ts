...
class WinButton extends UsesMember(FocusButton) {
    ...
    constructor(params: WinButtonParameters) {
        super(params);
        this.element.classList.value = (params.classList ?? ['win-button', 'icon-button']).join(
            ' ',
        );
        ...
        let r = await pointTransfer(
            {
                ...
                points:
                    getPointsFromFaan(faan) * (kind === 'baozimo' ? 3 : kind === 'dachut' ? 2 : 1), // baozimo triple from one, dachut double from one, zimo base amount from three players
                faan,
                win_kind: kind,
                datetime: new Date(),
...
interface PlayerTagParameters extends Omit<InputListenerParameters<'td'>, 'tag'> {
    tableNo: TableNo;
    seat: SeatWind;
    disabled?: boolean;
}
...
export default class PlayerTag extends UsesSeat(InputListener<'td'>) {
    ...
    constructor({ disabled = false, ...params }: PlayerTagParameters) {
        ...
        } else if (window.MJDATA.members.find((m) => m.id == this.memberId) === undefined) {
    ...
    generateListener(): EventListener {
        return async (ev) => {
            ...
            let newMember = window.MJDATA.members.find((v) => v.name === target.value);
