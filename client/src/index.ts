...
async function setup(data: Promise<MahjongData>) {
    ...
    for (let settingsButton of Array.from(document.getElementsByClassName('settings-button'))) {
...
function route() {
    ...
    let navButtons = Array.from(navbar.children).filter((v) => v.className.includes('nav-link'));
